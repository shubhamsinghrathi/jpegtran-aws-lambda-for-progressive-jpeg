...
exports.handler = function(event, context){
  var input_file_path = event.Records[0].s3.bucket.name;
  var input_file_name = decodeURIComponent(event.Records[0].s3.object.key.replace(/\+/g, " "));

  var full_path = '/'+ input_file_path +'/'+input_file_name;
  //imagemin([array of file paths], destination folder, OPTIONS).then(function(){});
  imagemin([full_path], '/'+input_file_path, {
    plugins: [
      imageminJpegtran(),
      imageminPngquant({quality: '65-80'})
    ]
  }).then(function(files){
    console.log("compressed file :",full_path, " to ", input_file_name);
    // console.log(files);
